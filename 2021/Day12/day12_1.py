...
def count_paths(ad_mat, names, current_cave):
    n_paths = 0
        ...
        if ad_mat[o][d] == 1:
            if d == len(ad_mat)-1:
                ...
                if current_cave.islower():
                    n_paths += count_paths(clear_col(ad_mat, o), names, names[d])
                else:
                    n_paths += count_paths(ad_mat, names, names[d])
    return n_paths


n = count_paths(adjacency_matrix, cave_names, 'start')
